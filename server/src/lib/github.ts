...
export async function addLabelsToRepository(
   ...
   * Helper functions
   */
  async function addLabelToRepository(label: GithubLabel): Promise<GithubLabel> {
    try {
      ...
      throw new Error(`Couldn't create ${label.name} in ${owner}/${repo}: ${err.message}`)
    }
  }
}
...
export async function updateLabelsInRepository(
   ...
   * Helper functions
   */
  async function updateLabelInRepository(label: GithubLabel): Promise<GithubLabel> {
    try {
      ...
      throw new Error(`Couldn't update ${label.name} in ${owner}/${repo}: ${err.message}`)
    }
  }
}
...
export async function removeLabelsFromRepository(
   ...
   * Helper functions
   */
  async function removeLabelFromRepository(label: GithubLabel): Promise<Octokit.IssuesDeleteLabelParams> {
    try {
      ...
      throw new Error(`Couldn't remove ${label.name} in ${owner}/${repo}: ${err.message}`)
    }
  }
}
...
export async function aliasLabelsInRepository(
  ...
  async function handler() {
    ...
    for (const issue of issues) {
      ...
      await addLabelsToIssue(github, { repo, owner, issue: issue.number }, missingLabels, persist)
    }
...
export function isLabel(local: GithubLabel): (remote: GithubLabel) => boolean {
  return (remote) =>
    local.name === remote.name && local.description === remote.description && local.color === remote.color
}

/**
 * Determines whether the two configurations configure the same label.
 */
export function isLabelDefinition(local: GithubLabel): (remote: GithubLabel) => boolean {
...
export async function getRepo(
  ...
  owner: string,
  repo: string,
): Promise<{ status: 'Exists'; repo: Octokit.ReposGetResponse } | { status: 'Unknown' }> {
  return github.repos
    .get({
      owner: owner,
      repo: repo,
 ...
 * trees on folders.
 */
async function createGhTree(github: Octokit, { owner, repo }: GHRepo, tree: GHTree): Promise<{ sha: string }> {
  /**
   * Uploads blobs and creates subtrees.
   */
  const blobs = await mapEntriesAsync(getTreeFiles(tree), (content) => createGhBlob(github, { owner, repo }, content))
 ...
 * @param repos
 */
export async function checkInstallationAccess(github: Octokit, configRepos: string[]): Promise<InstallationAccess> {
  /* istanbul ignore if */
  ...
  async function handler() {
    const res = await github.apps.listRepos({ per_page: 100, page }).then((res) => res.data)
